...
foursquares_red = foursquares.map(lambda f: (f['user']['id_str'], f['user']['name'], f['user']['description'], f['lang'], f[
                                  'text'], f['created_at'].replace("+0000 ", ""), f['geo']['coordinates'][0], f['geo']['coordinates'][1], f['place']['full_name']))
features = ['user_id', 'user_name', 'user_descr', 'language',
            'tweet_text', 'tweet_time', 'lat', 'lon', 'city']
...
foursquares_temp = foursquares_red.map(lambda p: (p[0], p[1], p[2], p[3], p[
                                       4], datetime.strptime(str(p[-4]), '%a %b %d %H:%M:%S %Y'), p[-3], p[-2], p[-1]))
foursquares_df = sqlContext.createDataFrame(foursquares_temp, schema)
foursquares_df.write.format("com.databricks.spark.csv").save(
